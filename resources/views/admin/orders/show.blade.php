...
@section('content')
    ...
    <div class="py-3 py-md-4">
        <div class="container">
                ...
                <div class="col-md-12">
                    <div class="shadow bg-white p-3">
                        <h4>
                            ...
                            <a href="{{ route('orders.invoice.generate', $order->id) }}"
                                class="btn btn-primary btn-sm float-end text-white">Download Invoice</a>
                            <a href="{{ route('orders.invoice.view', $order->id) }}" target="_blank"
                                class="btn btn-warning btn-sm float-end text-white">View Invooice</a>
                        </h4>
                        <hr />
                        <div class="row">
                            <div class="col-md-6">
                            ...
                            <table class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td colspan="5" class="fw-bold bg-primary text-white">
                                            ${{ $totalProductAmount }} </td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
            ...
            <div class="shadow bg-white p-3 py-md-4">
                <div class="row">
                    <div class="col-md-6">
                            ...
                            <div class="input-group">
                                <select name="status" id="status" class="form-select">
                                    ...
                                    <option value="In_Process"
                                        {{ $order->status_message == 'In_Process' || Request::get('status') == 'In_Process' ? 'selected' : '' }}>
                                        In Process</option>
                                    <option value="Completed"
                                        {{ $order->status_message == 'Completed' || Request::get('status') == 'Completed' ? 'selected' : '' }}>
                                        Completed</option>
                                    <option value="Pending"
                                        {{ $order->status_message == 'Pending' || Request::get('status') == 'Pending' ? 'selected' : '' }}>
                                        Pending</option>
                                    <option value="Cancelled"
                                        {{ $order->status_message == 'Cancelled' || Request::get('status') == 'Cancelled' ? 'selected' : '' }}>
                                        Cancelled</option>
                                    <option value="Out_For_Delivery"
                                        {{ $order->status_message == 'Out_For_Delivery' || Request::get('status') == 'Out_For_Delivery' ? 'selected' : '' }}>
                                        Out For Delivery</option>
                                </select>

                                <button type="submit" class="form-control btn btn-primary text-white"> Update Status
                                </button>
                            </div>
                        </form>
                    </div>
                    <div class="col-md-6 border">
                        <br />
                        <h4> Current Order Status: <span class="text-uppercase "> {{ $order->status_message }} </span>
                        </h4>
                    </div>
                </div>
            </div>
